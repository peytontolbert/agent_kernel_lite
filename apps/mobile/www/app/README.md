...
The v6 app install includes no default-installed extensions. The official
catalog in `web/extensions/catalog.json` is empty until an Agent Kernel Lite
extension is release-ready. Image Generation remains a development/future
extension and is not advertised by the official catalog. The app shell also
constrains fetched catalog entries to the pinned release allowlist so a stale
release catalog cannot re-advertise development extensions.

Mobile Computer Use is maintained as a standalone application:
...
The iOS and Android app now has a scaffold in `../apps/mobile`. It treats the
browser app as the source of truth by copying `web/` into
`apps/mobile/www/app/`, then using a small native boot shell for release checks
and asset caching. GitHub Releases provide executable assets and hashes; Hugging
Face provides model and paper data packs.

The native app should treat this JSON session file as the first import format.
The native shell can keep the same kernel concepts while replacing
